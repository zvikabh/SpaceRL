...
def ppo_update_loop(
  ...
  with timing.Timer('Data collection', disable=not show_timing_info):
  ...
  # Optimize
  with timing.Timer('Training', disable=not show_timing_info):
    for epoch in range(EPOCHS_PER_BATCH):
      ...
      for minibatch_start in range(0, len(all_steps), MINIBATCH_SIZE):
        ...
        mb_action_logits, mb_state_value = network(mb_states)
        mb_action_dist = Categorical(logits=mb_action_logits)
...
def main():
  ...
  parser.add_argument(
    '--show_timing_info', action='store_true', default=False,
    help='Show detailed wall time information per batch'
  )
  ...
  for i in range(start_iteration, 1000):
    ...
    ppo_update_loop(
      ...
      batch_num=i,
      save_best_episode=save_episode,
